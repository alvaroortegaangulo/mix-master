...
def _apply_multiband_eq_fft(
    ...
    if x.ndim == 1:
        ...
        n_ch = x.shape[1]
        # Procesamos cada canal en paralelo
        tasks = [
            (x[:, ch].copy(), sr, eq_gains_db, bands)  # copiamos para aislar memoria
...
def _eq_channel_worker(args: tuple[np.ndarray, int, Dict[str, float], Any]) -> np.ndarray:
    """
...
def _apply_multiband_eq_fft_channel(
    ...
    # Pequeño clamp para evitar clipping extremo
    y = np.clip(y, -1.5, 1.5)
...
def main() -> None:
    ...
    Stage S7_MIXBUS_TONAL_BALANCE:
      ...
      - Recalcula el tonal balance y guarda métricas en
        tonal_metrics_S7_MIXBUS_TONAL_BALANCE.json.
    """
    if len(sys.argv) < 2:
    ...
    # Si las bandas/targets vienen vacíos, reconstruimos objetivo desde estilo
    if not target_band_db:
        target_band_db = get_style_tonal_profile(style_preset)

    # Si el error RMS ya está por debajo del umbral, no hacemos nada
    MARGIN_RMS = 0.25
    if prev_error_rms <= max_tonal_error_db + MARGIN_RMS:
        ...
        # Aun así guardamos métricas mínimas
        _save_tonal_metrics(
            temp_dir=temp_dir,
    ...
    # 1) Calcular ganancia por banda para reducir error, acotada por max_eq_change_db
    errors_by_band, _ = compute_tonal_error(current_band_db, target_band_db)
    eq_gains_db: Dict[str, float] = {}

    for band_id, err_db in errors_by_band.items():
        # err_db = current - target; queremos mover hacia target
        desired_gain = -err_db  # si está +3 dB por encima, queremos -3 dB
        # limitamos a ±max_eq_change_db
        gain = max(-max_eq_change_db, min(max_eq_change_db, desired_gain))
        # si es muy pequeño, lo consideramos 0 para evitar microajustes
        if abs(gain) < 0.1:
            gain = 0.0
    ...
    # 3) Recalcular tonal balance post-EQ
    ...
    # 4) Guardar métricas para el futuro check
    _save_tonal_metrics(
        temp_dir=temp_dir,
        contract_id=contract_id,
        style_preset=style_preset,
