...
def sanitize_bus_cfg_for_mixing(
    ...
    """
    Versión "solo reverb" ultra conservadora.

    - Todos los buses: SOLO reverb (sin delays creativos ni modulación).
      El único Delay que se permite es el de pre-delay (feedback=0) antes
      de la reverb.
    - Drums / bass:
        * Envíos aún más secos que el resto.
        * Filtros HP/LP estrictos para que la cola no se coma el low-end.
    """
    cfg = replace(bus_cfg)
    key = bus_key
    ...
    # Aún más secos en drums y bass
    if key == "drums":
        # No permitir que suba por encima de -24 dB
        send_max = min(send_max, -24.0)
    elif key == "bass":
        # Bass todavía más discreto, máximo -32 dB
        send_max = min(send_max, -32.0)
    ...
    # -------------------- 4) FORZAR SOLO REVERB --------------------
    # Nada de delays creativos post-reverb
    cfg.delay_ms = None
    cfg.delay_feedback = 0.0

    # Nada de chorus/phaser en la cola
    cfg.mod_type = None
    cfg.mod_rate_hz = 0.0
    cfg.mod_depth = 0.0
...
def apply_audio_driven_adjustments(
    ...
    """
    Ajuste muy sutil del bus según el contenido, manteniendo el modo "solo reverb".

    - Pequeños cambios de send según crest.
    - Ajuste suave de HP/LP según graves/agudos presentes.
    - Tempo/estilo afectan sólo un poco.
    - Al final siempre pasa por sanitize_bus_cfg_for_mixing, que:
        * fuerza solo reverb (sin delays creativos ni modulación),
        * hace drums y bass muy secos.
    """
    cfg = replace(bus_cfg)
    crest = stats.crest
...
def build_space_pedalboard(bus_cfg: BusDefinition) -> Pedalboard:
    ...
    Filosofía "pro" en modo SOLO REVERB:
      ...
      - Pre-delay opcional antes de la reverb.
      - Nada de delays creativos ni modulación sobre la cola.
    """
    plugins = []

    # Filtros previos de la reverb
    ...
    # Pre-delay antes de la reverb (solo time shift, feedback=0)
    ...
    # IMPORTANTE: No se añaden delays creativos ni modulación aquí.
    # Delay creativo y Chorus/Phaser se han deshabilitado en sanitize_bus_cfg_for_mixing.
...
def _render_bus_fx_stem(
    ...
    # 1) Ajuste a tempo (incluye solo pre-delay en esta versión)
    tempo_notes: Dict[str, str] = {}
    cfg_for_tempo, tempo_notes = apply_tempo_to_bus_times(bus_cfg, tempo_bpm)

    # 2) Analizar contenido y ajustar (si está activado)
    ...
    if enable_audio_adaptive:
        ...
        final_cfg = apply_audio_driven_adjustments(cfg_for_tempo, stats, bus_key, tempo_bpm)

    # 3) Forzar límites (incl. solo reverb) una vez más por seguridad
    final_cfg = sanitize_bus_cfg_for_mixing(final_cfg, bus_key)
...
def _render_bus_fx_stem_from_cfg(
    ...
    stem_paths: List[Path],
    output_path: Path,
) -> Tuple[float, float, BusDefinition]:
    """
    Renderiza un stem de BUS FX 100 % wet usando una configuración YA calculada
    (por el análisis previo). No hace análisis ni tempo.

    En esta versión, se fuerza siempre:
      - mezcla SOLO reverb (sin delays creativos, sin chorus/phaser),
      - drums y bass con envíos muy secos y bien filtrados.

    Devuelve:
      peak_dbfs, rms_dbfs, bus_cfg_final
    """
    if not stem_paths:
        raise ValueError(f"No hay stems para el bus {bus_key}")
    ...
    # Forzar configuración segura para mezcla (solo reverb, límites por bus)
    cfg = sanitize_bus_cfg_for_mixing(bus_cfg, bus_key)

    board = build_space_pedalboard(cfg)

    wet = board(bus_dry, samplerate)

    send_gain = db_to_linear(cfg.send_level_db)
    wet *= send_gain
    ...
    logger.info(
        "[SPACE_DEPTH] Bus FX %s -> %s | peak=%.2f dBFS | rms=%.2f dBFS | send=%.2f dB",
        bus_key,
        output_path.name,
        peak_dbfs,
        rms_dbfs,
    ...
    return peak_dbfs, rms_dbfs, cfg


# ---------------------------------------------------------------------------
# Entry point de stage: run_space_depth (CORRECCIÓN)
# ---------------------------------------------------------------------------
...
def run_space_depth(
    ...
    En esta versión se aplica SIEMPRE un campo de solo reverb:
      - Sin delays creativos post-reverb.
      - Sin chorus/phaser.
      - Buses de drums y bass muy secos y filtrados en la cola.
    ...
    for row in payload:
        ...
        # En modo solo reverb, ignoramos los delay creativos y modulación
        ...
        try:
            peak_dbfs, rms_dbfs, effective_cfg = _render_bus_fx_stem_from_cfg(
                bus_key=bus_key,
                bus_cfg=bus_cfg,
        ...
        log_rows.append(
            {
                ...
                "peak_dbfs": peak_dbfs,
                "rms_dbfs": rms_dbfs,
                "send_level_db": effective_cfg.send_level_db,
                "reverb_archetype": effective_cfg.reverb_archetype,
                "pre_delay_ms": effective_cfg.pre_delay_ms,
                "hp_hz": effective_cfg.hp_hz,
                "lp_hz": effective_cfg.lp_hz,
            }
        )

        logger.info(
            "[SPACE_DEPTH] Bus '%s' procesado -> %s (peak=%.2f dBFS, rms=%.2f dBFS, send=%.2f dB)",
            bus_key,
            output_path.name,
            peak_dbfs,
            rms_dbfs,
            effective_cfg.send_level_db,
        )
